...
class JointEstimator:
  ...
  def calculate_angles(self):
    ...
    a2 = np.arccos(np.dot(y1, vx) / (np.linalg.norm(y1) * np.linalg.norm(vx)))
    
    a3 = np.arccos(np.dot(link2, vy) / (np.linalg.norm(link2) * np.linalg.norm(vy))) - (np.pi / 2)
    
    c = np.arcsin(np.linalg.norm(np.cross(link2, link3)) / (np.linalg.norm(link2) * np.linalg.norm(link3)))
    a4 = np.arccos(np.dot(link2, link3) / (np.linalg.norm(link2) * np.linalg.norm(link3))) * (-1 if c < 0 else 1)
